...
##### 后续+++
>后续又试了10000和100000随机数填充数组，对比了`for`循环和`stream.foreach`的速度，发现相差不是特别大（图就不贴了，大概是2400对比2500的样子），通过网上查找，`Stream`是用多线程处理的（该点待续，还没有彻底搞明白），所以之前基数少的时候创建多线程会耗时更多